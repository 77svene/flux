...
- `src/flux/` - Main package directory
...
- `src/train.py` - Training entry script (delegates to `flux.train.tuner`)
- `src/webui.py` - Web UI entry script (delegates to `flux.webui.interface`)
- `src/api.py` - API server entry script (delegates to `flux.api.app`)
- `tests/` - Test suite
...
- `src/flux/v1/` - Version 1 package directory
...
- Hyperparameters are defined using dataclasses in `src/flux/hparams/`

### Model Support

- New model support is added through model patches in `src/flux/model/`
- Visual models use the visual utilities in `src/flux/model/model_utils/visual.py`
- Quantization support is in `src/flux/model/model_utils/quantization.py`

### Data Processing

- Dataset definitions are in `data/dataset_info.json`
- Data templates and processors are in `src/flux/data/`

### Training

- Training pipelines are in `src/flux/train/`
...
- **CLI Training**: `flux-cli train --config examples/train_lora/llama3_lora_sft.yaml`
- **Web UI**: `flux-cli webui` or `python src/webui.py`
- **API Server**: `flux-cli api` or `python src/api.py`
- **Chat Interface**: `flux-cli chat --model_name_or_path MODEL_PATH`
